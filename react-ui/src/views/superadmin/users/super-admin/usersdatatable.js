...
const HelloWorld = () => {
  ...
  const handleDownloadLimenealScores = async (user_id) => {
    const payload = { user_id };

    const res = await Axios.post('superadmin/download_scores/', payload,{
      responseType:'blob',
    });

    console.log(res.headers['file-name'])
    const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      console.log(res)
      link.setAttribute('download', 'LimenealScores.xlsx');
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  };
  const rowExpansionTemplate = (data) => {
    ...
    return (
      <div className="p-3">
        ...
        <Button
          onClick={() => handleDownloadLimenealScores(data.id)}
          variant="outlined"
        ...
        <DataTable
          tableStyle={{ fontSize: '1em' }}
          style={{ fontSize: '1em' }}
          value={data.allowed_reports}
        ...
        >
          ...
          <Column
            ...
            header="Level - Report Type"
            body={(rowData) => `${rowData.level} - ${rowData.report_type}`}
          />
  ...
  return (
    <div>
      ...
      >
        <Column
          expander={(e) => {
            return e.allowed_reports.length;
          }}
          style={{ width: '5rem' }}
        />
